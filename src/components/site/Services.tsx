...
const SERVICES = [
  {
    icon: "/images/icons/Remodeling.svg",
    number: "01",
    title: "Remodeling",
    description:
  ...
  {
    icon: "/images/icons/MEP Services.svg",
    number: "03",
    title: "MEP Services",
    description:
  ...
  {
    icon: "/images/icons/ExteriorStructures.svg",
    number: "04",
...
export function Services() {
  return (
    <section id="services" className="relative w-full overflow-hidden bg-brand-charcoal py-24">
...
function ServiceCard({
  ...
  return (
    <div className="group relative h-full border border-white/15 bg-brand-charcoal p-6 pt-8 transition-colors duration-200 hover:border-brand-yellow">
      <div className="absolute -top-3.5 left-6 flex h-7 w-7 items-center justify-center bg-white/10 text-xs font-black text-neutral-300 transition-colors duration-200 group-hover:bg-brand-yellow group-hover:text-black">
        {number}
      </div>
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-neutral-400 transition-colors duration-200 hover:text-white"
      >
      ...
      >
        <ul className="overflow-hidden border-t border-white/15 pt-3">
          {items.map((item) => (
            <li key={item} className="flex items-start gap-2 py-1 text-lg text-neutral-300">
      ...
      <CtaButton href="#contact" tone="dark" className="mt-6 w-full">
        Submit a Request
      </CtaButton>
    </div>
  );
